...
real learn_rate_mode = 1.0; // current iteration number
...
real eta = 0.001; // Initial learning rate
...
void *glove_thread(void *vid) {
    ...
    for(a = 0; a < lines_per_thread[id]; a++) {
        ...
        cost[id] += isnan(0.5 * fdiff * diff)? 0 : 0.5 * fdiff * diff; // weighted squared error

        // printf("%f\n", cost[id]);
    
        /* Adaptive gradient updates */
        fdiff *= eta/learn_rate_mode; // for ease in calculating gradient
        for(b = 0; b < vector_size; b++) {
            // learning rate times gradient for word vectors
            temp1 = isnan(fdiff * W[b + l2])? 0 : fdiff * W[b + l2];
            temp2 = isnan(fdiff * W[b + l1])? 0 : fdiff * W[b + l1];
            // adaptive updates
        ...
        fdiff = isnan(fdiff*fdiff)? 0 : fdiff*fdiff;
...
int save_params() {
    long long a, b;
    char format[20];
    char output_file[MAX_STRING_LENGTH],output_file2[MAX_STRING_LENGTH], output_file_gsq[MAX_STRING_LENGTH];
    ...
    if(use_binary == 2) {
        FILE *fout1, *fout2;
        sprintf(output_file,"%s_1.txt",save_W_file);
        sprintf(output_file2,"%s_2.txt",save_W_file);
        fout1 = fopen(output_file,"w");
        fout2 = fopen(output_file2,"w");
        long long i, j;
        for(i = 0; i < 2*vocab_size; i++) {
            for(j = 0; j < vector_size+1; j++) {
                if(i >= vocab_size) {
                    if(j == vector_size) {
                        ...
                        fprintf(fout2, "%lf ", W[(vector_size+1)*i+j]);
                    }
                } else {
                    if(j == vector_size) {
                        fprintf(fout1, "\n");
                        ...
                        fprintf(fout1, "%lf ", W[(vector_size+1)*i+j]);
                    }
                }
            }
        }
    ...
    if(use_binary == 3) {
        ...
        real tmp;
        long long i, j;
        sprintf(dir,"./vectors_%d",num_iter);
        struct stat st = {0};
        if (stat(dir, &st) == -1) {
            mkdir(dir, 0700);
        }
        for(i = 0; i < vocab_size; i++) {
            sprintf(output_file,"./vectors_%d/%lld.bin",num_iter,i);
            ...
            for(j = 0; j < vector_size+1; j++) {
                ...
                tmp = W[(vector_size+1)*(i+vocab_size)+j] + W[(vector_size+1)*i+j];
                fprintf(fout, "%lf ", tmp);
            }
        }
    }
